...
<xsl:stylesheet version="1.0"
  ...
  exclude-result-prefixes="xsl tei xd eg fn #default">
	...
	<xsl:param name="customCSS" select="concat($filePrefix,'/css/item-markup.css')"/>
	<xsl:param name="emanCSS" select="concat($filePrefix,'/css/item-clear.css')"/>
	...
	<xsl:template name="teibpToolbox">
		<div id="teibpToolbox">
			<label for="showMarkup">Montrer le balisage</label>
			<input type="checkbox" id="showMarkup" /> <br />
			<label for="pleinecran">Plein ecran</label>
			<input type="checkbox" id="pleinecran" /> 
		</div>
	</xsl:template>
  ...
  <xsl:template match="tei:figure[tei:graphic[@url]]" priority="99">
   ...
   <script type="text/x-mathjax-config">
			...
			<link id="emancss" rel="stylesheet" type="text/css" href="{$emanCSS}"/>			
