...
//     Zend_Debug::dump($options);
  ...
  public function exportAction() {  
    ...
    $options = array();
    $options = get_option('transcript_options', array()) . '||||' . get_option('transcript_controle', array());    
    echo $options;
    $this->view->layout()->disableLayout();
    $this->_helper->viewRenderer->setNoRender(true);    
    $this->view->content = "";    
...
/*
    ...
    $xsl_doc->load(BASE_DIR . '/plugins/Transcript/tei2html/tei2html.xsl');    
    $proc = new XSLTProcessor();
    $proc->importStylesheet($xsl_doc);
    $newdom = $proc->transformToDoc($doc);
    
    $transcriptionText = $newdom->saveXML();
*/
...
//     $this->view->niveaux = $options['niveaux'];
    ...
    $this->view->imgheight = $size[1];    
		...
		$this->view->pager = files_pager('/transcribe/');
	}
