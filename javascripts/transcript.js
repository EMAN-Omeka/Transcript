...
jQuery(document).ready(function() {
  ...
  tinymce.init({
    ...
    toolbar1: 'undo redo | insert | gras italique souligne | pleft pcenter pright pjustify | linebreak | mathml | code',
...
function add_valid_elements() {
	var valid_elements = 'del[rend|class],i[class|title],math[xmlns],span[class],xml:id';
	...
	valid_elements = valid_elements + ',ttable,ffigure,l[xml::id|n]';
       ...
       editor.windowManager.open({
	        ...
	        onsubmit: function(e) {
             ...
             tagName = button.cl;
  		 				...
  		 				if (tagName == "hi") {
    		 				if (couleur != null) {
                  n = attributes.lastIndexOf('"');
                  attributes = attributes.substring(0,n) + ' ' + couleur + attributes.substring(n);
                }
    		 			}
 		 					tinymce.activeEditor.selection.setContent("<" + tagName + " " + attributes + " class='" + attrValue + " " + tagName + " " + couleur + "' data='" + visu[button.cl] + "'><i class='fa fa-fw fa-" + icon + " fa-border green inv'>&nbsp;</i> " + selection + "</" + tagName + ">");
		 				});
	        }
...
function add_form_element(button) {
  ...
  form_element2 =  '';
	...
	// Couleur
  if (button.att == 'rend') {
   	form_element3 = {type: 'listbox', name: 'couleur', label: 'Couleur', values: [{text: 'none', value: null}, {text: 'noir', value: 'black'}, {text: 'rouge', value: 'red'}, {text: 'vert', value:'green'}, {text: 'jaune', value:'yellow'}, {text: 'violet', value:'purple'}]};
  } else {
    form_element3 = '';
  }
	form = [form_element, form_element2, form_element3];
	return form;
}